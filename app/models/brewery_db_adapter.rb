class BreweryDBAdapter
  ...
  def beers(styleId)
    self.class.get("/beers", { query: { key: ENV['BREWERY_DB_ACCESS_TOKEN'], styleId: styleId, order: "random", randomCount: 10 }} )['data'].map { |args| Beer.new(database_id: args['id'], name: args['name'], abv: args['abv'], style_id: args['styleId'], description: args['description']) }
  end
  def find_beer_by_id(database_id)
    beer_data = self.class.get("/beer/#{database_id}", @options)['data']
        # , glassware: beer_data['glass']['name'], available: beer_data['available']['name'], available_description: beer_data['available']['description']

    Beer.new(database_id: beer_data['id'], name: beer_data['name'], abv: beer_data['abv'], style_id: beer_data['styleId'], description: beer_data['description'])
  end
end
